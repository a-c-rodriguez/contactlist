...
public class ContactStoreImpl implements IContactStore {
    ...
    // private ArrayASTBinarySearchTree<Contact> contacts = new
        ...
        return contacts.stream()
                .filter(contact -> {
                    if (null == contact)
                        return false;
                    if (findContact.id > 0) {
                        // System.out.println(MessageFormat.format("contact.id {0} and findContact.id
                        // {1}", contact.id, findContact.id));
                        ...
                        boolean matchFirstName = (!hasFirstName) ? true
                                : Objects.equals(findContact.firstName, contact.firstName);
                        boolean matchLastName = (!hasLastName) ? true
                                : Objects.equals(findContact.lastName, contact.lastName);
                        boolean matchStreetAddress = (!hasStreetAddress) ? true
                                : Objects.equals(findContact.streetAddress, contact.streetAddress);
                        ...
                        boolean matchPostalCode = (!hasPostalCode) ? true
                                : Objects.equals(findContact.postalCode, contact.postalCode);

                        return matchFirstName && matchLastName && matchStreetAddress && matchCity && matchState
                                && matchPostalCode;
                    }
                })
