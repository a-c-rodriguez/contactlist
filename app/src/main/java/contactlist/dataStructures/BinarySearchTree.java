...
public class BinarySearchTree<T extends Comparable<T>> implements Collection<T> {
    ...
    public void addNode(T value) {
        addNode(new BinarySearchTreeNode<T>(value));
    }

    private void addNode(BinarySearchTreeNode<T> node) {
        if (null == this.rootNode) {
            this.rootNode = node;
            this.count++;
        } else {
            insertNode(this.rootNode, node);
        }
    }

    private void insertNode(BinarySearchTreeNode<T> parentNode, BinarySearchTreeNode<T> node) {
        if (parentNode == null) {
            parentNode = node;
            this.count++;
        } else {
    ...
    public void deleteNode(Object o) {
        ...
        if (null != val)
            deleteNode(new BinarySearchTreeNode<T>(val));
    }

    public void deleteNode(T value) {
        deleteNode(new BinarySearchTreeNode<T>(value));
    }

    private void deleteNode(BinarySearchTreeNode<T> node) {
        removeNode(new BinarySearchTreeNodeWithParent<>(this.rootNode, this.rootNode), node);
    }

   private void removeNode(BinarySearchTreeNodeWithParent<T> nodeWithParent, BinarySearchTreeNode<T> node) {
        BinarySearchTreeNodeWithParent<T> foundNode = findNode(nodeWithParent, node); 
        if(foundNode == null || foundNode.childNode == null) {
            contactlist.App.print("No match foundNode for " + node.data.toString());
            return;
        } else {
            BinarySearchTreeNode<T> found = foundNode.childNode;
            BinarySearchTreeNode<T> parentNode = foundNode.parentNode;
            //zero child - delete
            if(null == found.leftNode && null == found.rightNode) {
                contactlist.App.print("Set node to false for node " + found.data.toString());
                if(parentNode != null) {
                    if(parentNode.leftNode.data.compareTo(found.data) == 0){
                        parentNode.leftNode = null;
                    } else if(parentNode.rightNode.data.compareTo(found.data) == 0) {
            ...
            //two children - goto right child and promote left most child
                ...
                found = temp;
                found.leftNode = foundLeft;
                found.rightNode = foundRight;
            } else {
                //one child - promote child
                BinarySearchTreeNode<T> child = (null == found.rightNode) ? found.leftNode : found.rightNode;
                contactlist.App.print("Promoted child node " + child.data.toString());
                if(parentNode != null) {
                    if(parentNode.leftNode.data.compareTo(found.data) == 0){
    ...
    public BinarySearchTreeNodeWithParent<T> search(Object o) {
        T val = null;
        if (o instanceof Comparable) {
            val = (T) o;
        }
        if (null != val) {
            return search(new BinarySearchTreeNode<T>(val));
        } else {
            return null;
        }
    }

    public BinarySearchTreeNodeWithParent<T> search(T value) {
        return search(new BinarySearchTreeNode<T>(value));
    }

    public BinarySearchTreeNodeWithParent<T> search(BinarySearchTreeNode<T> node) {
        return findNode(new BinarySearchTreeNodeWithParent<T>(this.rootNode, this.rootNode), node);
    }

    private BinarySearchTreeNodeWithParent<T> findNode(BinarySearchTreeNodeWithParent<T> nodeWithParent,
            BinarySearchTreeNode<T> nodeToFind) {
        if (nodeWithParent.childNode == null) {
            return null;
        } else {
            if (nodeToFind.data.compareTo(nodeWithParent.childNode.data) == 0) {
                return nodeWithParent;
            } else if (nodeToFind.data.compareTo(nodeWithParent.childNode.data) < 0) {
                return findNode(new BinarySearchTreeNodeWithParent<T>(nodeWithParent.childNode,
                        nodeWithParent.childNode.leftNode), nodeToFind);
            } else {
                return findNode(new BinarySearchTreeNodeWithParent<T>(nodeWithParent.childNode,
                        nodeWithParent.childNode.rightNode), nodeToFind);
            }
        }
    }

    private void InPlaceTraversal(List<T> list, BinarySearchTreeNode<T> node) {
        if (null != node) {
    ...
    public boolean contains(Object o) {
        BinarySearchTreeNodeWithParent<T> node = search(o);
        return null != node && null != node.childNode;
    }
