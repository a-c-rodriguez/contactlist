...
public class LinkedListImpl<T extends Comparable<T>> implements Collection<T> {
    ...
    private LinkedListNode<T> getHeadNode(){
        return head;
    }

    private void setHeadNode(LinkedListNode<T> head){
        this.head = head;
    }

    private LinkedListNode<T> getTailNode(){
        return tail;
    }

    private void setTailNode(LinkedListNode<T> tail){
        this.tail = tail;
    }
    ...
    public void addHead (LinkedListNode<T> node) {
        LinkedListNode<T> temp = getHeadNode();
        setHeadNode(node);
        getHeadNode().setNext(temp); 
        this.count++;
        if(1 == count) {
            setTailNode(getHeadNode());
        }
    }
    ...
    public boolean isEmpty(){
        return this.count == 0 && null == getHeadNode() && null == getTailNode();
    }
    ...
    public boolean contains(Object o) {
        LinkedListNode<T> node = getHeadNode();
        while(node != null){
    ...
    public Iterator<T> iterator(){
        return new LinkedListImplIterator<T>(getHeadNode()); 
    }

    private class LinkedListImplIterator<T extends Comparable<T>> implements Iterator<T>{
        LinkedListNode<T> current;
        LinkedListNode<T> lastReturned;

        public LinkedListImplIterator(LinkedListNode<T> head){
            current = head;
        }
    ...
    public <E> E[] toArray(E[] a){       
        ...
        int i = 0;
        Object[] result = a;
        for (LinkedListNode<T> x = getHeadNode(); x != null; x = x.getNext())
            result[i++] = x.getValue();
    ...
    public boolean remove(Object o){
        ...
        while(node != null) {
            if(node.getValue().equals(o)){
                ...
                if(prev == null) { //head node removal
                    setHeadNode(next);
                    node.setNext(null);
                    node = null;
                    this.count--;
                ...
                node.setNext(null);
                node = null;
                if(prev.getNext() == null){ //tail node removal
                    setTailNode(prev);
                }
                this.count--;
                return true;
